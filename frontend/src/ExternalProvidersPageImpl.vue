...
type Model = { id:string; providerModelId:string; displayName:string; compatibilityKey?:string; contextLength?:number; maxConcurrency:number; capabilitiesJson:string; healthStatus:string; enabled:boolean; inputPricePerMillion?:number; outputPricePerMillion?:number; currency?: 'KRW' | 'USD' }
...
const modelForm = ref({ providerModelId:'', displayName:'', compatibilityKey:'', contextLength:128000, maxConcurrency:20, capabilitiesJson:'[]', inputPricePerMillion:0, outputPricePerMillion:0, currency:'KRW' as 'KRW' | 'USD', enabled:true })
...
function resetModelForm() { modelForm.value={providerModelId:'',displayName:'',compatibilityKey:'',contextLength:128000,maxConcurrency:20,capabilitiesJson:'[]',inputPricePerMillion:0,outputPricePerMillion:0,currency:'KRW',enabled:true} }
function openModel(item?:Model) { editingModel.value=item??null; if (item) modelForm.value={providerModelId:item.providerModelId,displayName:item.displayName,compatibilityKey:item.compatibilityKey??'',contextLength:item.contextLength??128000,maxConcurrency:item.maxConcurrency??20,capabilitiesJson:item.capabilitiesJson??'[]',inputPricePerMillion:item.inputPricePerMillion??0,outputPricePerMillion:item.outputPricePerMillion??0,currency:item.currency??'KRW',enabled:item.enabled}; else resetModelForm(); modelOpen.value=true }
