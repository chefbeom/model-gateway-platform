...
type Model = { id:string; providerModelId:string; displayName:string; compatibilityKey?:string; contextLength?:number | null; maxConcurrency:number; capabilitiesJson:string; healthStatus:string; enabled:boolean; inputPricePerMillion?:number | null; outputPricePerMillion?:number | null; currency?: 'KRW' | 'USD' }
type ModelForm = { providerModelId:string; displayName:string; compatibilityKey:string; contextLength:number|null; maxConcurrency:number; capabilitiesJson:string; inputPricePerMillion:number|null; outputPricePerMillion:number|null; currency:'KRW'|'USD'; enabled:boolean }
...
const modelForm = ref<ModelForm>({ providerModelId:'', displayName:'', compatibilityKey:'', contextLength:null, maxConcurrency:20, capabilitiesJson:'[]', inputPricePerMillion:null, outputPricePerMillion:null, currency:'USD', enabled:true })
...
function resetModelForm() { modelForm.value={providerModelId:'',displayName:'',compatibilityKey:'',contextLength:null,maxConcurrency:20,capabilitiesJson:'[]',inputPricePerMillion:null,outputPricePerMillion:null,currency:'USD',enabled:true} }
function openModel(item?:Model) { editingModel.value=item??null; if (item) modelForm.value={providerModelId:item.providerModelId,displayName:item.displayName,compatibilityKey:item.compatibilityKey??'',contextLength:item.contextLength??null,maxConcurrency:item.maxConcurrency??20,capabilitiesJson:item.capabilitiesJson??'[]',inputPricePerMillion:item.inputPricePerMillion??null,outputPricePerMillion:item.outputPricePerMillion??null,currency:item.currency??'KRW',enabled:item.enabled}; else resetModelForm(); modelOpen.value=true }
function registerDiscoveredModel(item:ProbeModel) { const existing=models.value.find(model=>model.providerModelId===item.modelId); probeOpen.value=false; if(existing){openModel(existing);return} editingModel.value=null; modelForm.value={providerModelId:item.modelId,displayName:item.modelId,compatibilityKey:item.modelId,contextLength:item.contextLength??null,maxConcurrency:item.maxConcurrency??20,capabilitiesJson:JSON.stringify(item.capabilities??[]),inputPricePerMillion:null,outputPricePerMillion:null,currency:'USD',enabled:true}; modelOpen.value=true }
function setModelPrice(field:'inputPricePerMillion'|'outputPricePerMillion', event:Event) { const raw=(event.target as HTMLInputElement).value; modelForm.value[field]=raw.trim()===''?null:Number(raw) }
...
async function saveModel() { if (!selected.value || !modelForm.value.providerModelId || !modelForm.value.displayName) return; busy.value=true; try { const wasEditing=Boolean(editingModel.value); const path=wasEditing ? `/api/admin/external-providers/${selected.value.id}/models/${editingModel.value!.id}` : `/api/admin/external-providers/${selected.value.id}/models`; const body=wasEditing ? { displayName:modelForm.value.displayName, compatibilityKey:modelForm.value.compatibilityKey, contextLength:modelForm.value.contextLength, maxConcurrency:modelForm.value.maxConcurrency, capabilitiesJson:modelForm.value.capabilitiesJson, inputPricePerMillion:modelForm.value.inputPricePerMillion, outputPricePerMillion:modelForm.value.outputPricePerMillion, currency:modelForm.value.currency, enabled:modelForm.value.enabled } : { providerModelId:modelForm.value.providerModelId, displayName:modelForm.value.displayName, compatibilityKey:modelForm.value.compatibilityKey, contextLength:modelForm.value.contextLength, maxConcurrency:modelForm.value.maxConcurrency, capabilitiesJson:modelForm.value.capabilitiesJson, inputPricePerMillion:modelForm.value.inputPricePerMillion, outputPricePerMillion:modelForm.value.outputPricePerMillion, currency:modelForm.value.currency }; const saved=await adminFetch<Model>(path,props.auth,{method:wasEditing?'PATCH':'POST',body:JSON.stringify(body)}); models.value=wasEditing ? models.value.map(item=>item.id===saved.id?saved:item) : [...models.value,saved]; modelOpen.value=false; editingModel.value=null; message.value=wasEditing?'외부 모델 설정을 수정했습니다.':'외부 모델을 등록했습니다.' } catch (error) { showError(error, '외부 모델 설정 저장에 실패했습니다.') } finally { busy.value=false } }
