...
const overview = ref<Overview | null>(null); const endpoints = ref<Endpoint[]>([]); const incidents = ref<Incident[]>([]); const busy = ref(false); const error = ref('')
async function load() {
  ...
  try {
    ...
    overview.value = summary as Overview; incidents.value = (incidentItems ?? []) as Incident[]; endpoints.value = await adminFetch<Endpoint[]>('/api/admin/runtime-endpoints', props.auth)
