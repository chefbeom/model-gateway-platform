...
function openCreateService() {
  ...
  serviceForm.value = { serviceKey: '', displayName: '', failoverPolicy: 'STRICT', retryPolicy: 'SAFE', allowDegraded: false, requiredCapabilitiesJson: '[]', inputPricePerMillion: 0, outputPricePerMillion: 0, enabled: true }
...
function openEditService() {
  ...
  serviceForm.value = { ...selected.value }
  serviceModal.value = true
}
...
<template>
  <section class="page-stack">
    ...
    <div class="split-layout services-layout">
      ...
      <article class="surface-card detail-panel">
        <template v-if="selected">
          ...
          <div class="policy-summary"><div><span>Failover</span><strong>{{ selected.failoverPolicy }}</strong></div><div><span>Retry</span><strong>{{ selected.retryPolicy }}</strong></div><div><span>입력 단가 / 1M</span><strong>{{ selected.inputPricePerMillion }}</strong></div><div><span>출력 단가 / 1M</span><strong>{{ selected.outputPricePerMillion }}</strong></div></div>
    ...
    <BaseModal :open="deleteServiceModal" title="논리 서비스 삭제" description="삭제 가능 여부를 먼저 검사했습니다. 이 작업은 되돌릴 수 없습니다." size="sm" @close="deleteServiceModal = false">
      <div class="deletion-check" v-if="deletionCheck"><strong>{{ deletionCheck.displayName }}</strong><code>model: {{ deletionCheck.serviceKey }}</code><template v-if="deletionCheck.canDelete"><p>이 서비스에는 프로젝트 권한, Service Target, 요청 이력이 없어 안전하게 삭제할 수 있습니다.</p></template><template v-else><p class="danger-text">삭제 차단 사유를 항목별로 정리하세요. 프로젝트 자체를 삭제할 필요는 없습니다.</p><ul><li v-if="deletionCheck.projectAccessCount"><b>프로젝트 권한: {{ deletionCheck.projectAccessCount }}개</b><small>프로젝트 & API 키 → 서비스 권한에서 해당 논리 서비스의 권한을 해제하세요.<span v-if="deletionCheck.linkedProjectNames.length"> 대상: {{ deletionCheck.linkedProjectNames.join(', ') }}</span></small></li><li v-if="deletionCheck.targetCount"><b>Service Target: {{ deletionCheck.targetCount }}개</b><small>현재 서비스의 Target을 모두 제거하거나 다른 서비스로 이전하세요.</small></li><li v-if="deletionCheck.requestHistoryCount"><b>요청 이력: {{ deletionCheck.requestHistoryCount }}건</b><small>감사·사용량 이력은 삭제하지 않습니다. 이 항목이 있으면 영구 삭제 대신 정책 편집에서 서비스를 비활성화해 보관하세요.</small></li></ul></template></div>
      <template #footer><button class="secondary-button" @click="deleteServiceModal = false">{{ deletionCheck?.canDelete ? '취소' : '확인' }}</button><button v-if="deletionCheck?.canDelete" class="secondary-button danger-text" :disabled="busy" @click="deleteService">서비스 영구 삭제</button></template>
    </BaseModal>
  </section>
</template>
