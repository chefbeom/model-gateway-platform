...
type RequestItem = { requestId: string; projectId: string; serviceId: string; finalDeploymentId?: string; status: string; inputTokens?: number; outputTokens?: number; estimatedCost?: number; latencyMs?: number; failoverCount: number; providerType?: string; routingReason?: string; startedAt: string; attempts: Attempt[]; reasoningEffort?: string | null; requestedServiceTier?: string | null; actualServiceTier?: string | null }
...
<template>
  <section class="page-stack">
    ...
    <article v-if="tab === 'requests'" class="surface-card">
      ...
      <div v-if="requests.length" class="data-table-wrap"><table class="data-table request-table"><thead><tr><th>시간</th><th>상태</th><th>요청 ID</th><th>추론·tier</th><th>토큰</th><th>지연</th><th>Failover</th><th>Provider</th><th>Attempts</th><th></th></tr></thead><tbody><tr v-for="item in requests" :key="item.requestId"><td>{{ new Date(item.startedAt).toLocaleString() }}</td><td><span class="status-chip tiny" :class="item.status === 'SUCCEEDED' ? 'healthy' : item.status === 'IN_PROGRESS' ? 'unknown' : 'unhealthy'">{{ item.status }}</span></td><td class="mono clipped">{{ item.requestId }}</td><td>{{ formatExecutionMode(item.reasoningEffort,item.requestedServiceTier,item.actualServiceTier) }}</td><td>{{ (item.inputTokens ?? 0) + (item.outputTokens ?? 0) }}</td><td>{{ duration(item.latencyMs) }}</td><td><span :class="{ 'accent-text': item.failoverCount > 0 }">{{ item.failoverCount }}</span></td><td><span class="provider-route">{{ item.providerType ?? 'LOCAL' }}<small v-if="item.routingReason">{{ item.routingReason }}</small></span></td><td>{{ item.attempts.length }}</td><td><button class="text-button" @click="inspect(item)">상세</button></td></tr></tbody></table></div><div v-else class="empty-state"><span>◎</span><h3>조건에 맞는 요청이 없습니다</h3><p>필터를 변경하거나 API 요청을 실행해보세요.</p></div>
    </article>
