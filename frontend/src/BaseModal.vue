...
const helpByLabel: Record<string, string> = {
  ...
  'API Identifier': 'LM Studio에서 로드 인스턴스에 붙이는 API 별칭입니다. 현재 native REST에서는 프로필에 보관하고 Agent·CLI 연동 시 적용합니다.',
  '모델 파일 / 양자화 변형': '같은 모델의 Q4/Q5/Q6/Q8 등 다른 모델 파일입니다. LM Studio의 현재 선택 변형과 일치하는지 확인하세요.',
  'Evaluation Batch Size': '입력 토큰을 평가할 때 한 번에 처리하는 토큰 수입니다. 값을 키우면 속도와 메모리 사용량이 함께 증가할 수 있습니다.',
  'Physical Batch Size': '하드웨어에 맞춘 물리 배치 크기입니다. 현재 AICONNECT에서는 프로필에 저장되며 Node Agent·SDK 적용이 필요합니다.',
  'Max Concurrent Predictions': '동시에 처리할 수 있는 예측 수입니다. GPU 메모리와 실제 LM Studio 설정을 함께 확인하세요.',
  'MoE Expert 수': 'Mixture-of-Experts 모델에서 사용할 Expert 수입니다. 일반 모델에는 적용되지 않습니다.',
  'KV Cache GPU Offload': 'KV 캐시를 GPU 메모리에 둘지 선택합니다. GPU 메모리가 부족하면 CPU 저장을 고려하세요.',
  'GPU Offload': '모델 계산을 GPU에 얼마나 배치할지 정하는 옵션입니다. 현재 화면의 값은 CLI·SDK 또는 Node Agent 연동 시 적용됩니다.',
  'GPU Offload 비율 (0~1)': '0은 GPU 오프로딩 없음, 1은 전체 오프로딩을 의미합니다. 예: 0.8은 약 80%입니다.',
  'GPU Offload Layers': 'GPU로 옮길 레이어 수입니다. GPU 메모리 용량에 맞춰 조정하며 Agent·CLI 적용이 필요합니다.',
  'CPU Thread Pool Size': 'CPU에서 사용할 스레드 수입니다. CPU와 모델 엔진에 따라 최적값이 달라집니다.',
  '자동 언로드 TTL (초)': '마지막 요청 후 지정한 초가 지나면 모델을 메모리에서 내리는 시간입니다.',
  'Seed': '동일한 조건에서 재현 가능한 결과를 얻기 위한 난수 시드입니다.',
  'RoPE Frequency Base': 'RoPE 기본 주파수입니다. 모델 기본값을 모르면 비워 두는 것을 권장합니다.',
  'RoPE Frequency Scale': 'RoPE 주파수 배율입니다. 긴 컨텍스트 실험 시 모델 문서와 함께 조정하세요.',
  'K Cache Quantization': 'Attention K 캐시의 양자화 형식입니다. 메모리는 줄지만 품질에 영향을 줄 수 있습니다.',
  'V Cache Quantization': 'Attention V 캐시의 양자화 형식입니다. Flash Attention과 엔진 지원 여부를 확인하세요.',
  'Unified KV Cache': '여러 요청의 KV 캐시를 통합하는 엔진 옵션입니다. Agent·SDK 적용 항목입니다.',
  'Keep Model in Memory': '모델을 시스템 메모리에 계속 유지해 재로드를 줄이는 옵션입니다. RAM 사용량이 증가할 수 있습니다.',
  'mmap 사용': '모델 파일을 메모리 매핑으로 읽는 옵션입니다. 디스크와 RAM 상태에 따라 효과가 달라집니다.',
  ...
  'Retry 정책': '같은 요청을 다른 Target으로 재시도하는 범위입니다. SAFE도 컨텍스트·입출력 토큰 한도처럼 전송 전에 확인되거나 모델별로 거부된 요청은 호환 Target으로 전환하며, 연결 이후 응답이 시작된 요청은 재전송하지 않습니다. AGGRESSIVE는 일시적 5xx·타임아웃도 재시도합니다.',
...
const exampleByLabel: Record<string, string> = {
  ...
  'Flash Attention': 'ON',
  'API Identifier': 'gemma-4-12b-q8',
  '모델 파일 / 양자화 변형': 'Q8_0 · google/gemma-4-12b@q8_0',
  'Evaluation Batch Size': '512',
  'Physical Batch Size': '512',
  'Max Concurrent Predictions': '4',
  'MoE Expert 수': '4',
  'KV Cache GPU Offload': 'ON',
  'GPU Offload': 'Max 또는 0.8',
  'GPU Offload 비율 (0~1)': '0.8',
  'GPU Offload Layers': '48',
  'CPU Thread Pool Size': '8',
  '자동 언로드 TTL (초)': '3600',
  'Seed': '42',
  'RoPE Frequency Base': '모델 기본값',
  'RoPE Frequency Scale': '모델 기본값',
