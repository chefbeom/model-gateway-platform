...
const runtime = ref({ nodeName: '', runtimeName: '', description: '', baseUrl: 'http://gpu-node-01:1234', apiToken: '', inputPricePerMillion: 0, outputPricePerMillion: 0, currency: 'KRW' as 'KRW' | 'USD', clearPricing: true })
const endpointForm = ref({ displayName: '', baseUrl: '', enabled: true, apiToken: '', clearApiToken: false, inputPricePerMillion: 0, outputPricePerMillion: 0, currency: 'KRW' as 'KRW' | 'USD', clearPricing: false })
...
async function createRuntime() {
  ...
  try {
    ...
    const endpoint = await adminFetch<Endpoint>('/api/admin/runtime-endpoints', props.auth, { method: 'POST', body: JSON.stringify({ nodeId: node.id, displayName: runtime.value.runtimeName || runtime.value.nodeName, runtimeType: 'LM_STUDIO', baseUrl: runtime.value.baseUrl, apiToken: runtime.value.apiToken || null, inputPricePerMillion: runtime.value.clearPricing ? null : runtime.value.inputPricePerMillion, outputPricePerMillion: runtime.value.clearPricing ? null : runtime.value.outputPricePerMillion, currency: runtime.value.clearPricing ? null : runtime.value.currency }) })
    runtime.value = { nodeName: '', runtimeName: '', description: '', baseUrl: 'http://gpu-node-01:1234', apiToken: '', inputPricePerMillion: 0, outputPricePerMillion: 0, currency: 'KRW' as 'KRW' | 'USD', clearPricing: true }
...
async function openEndpointSettings(endpoint = selected.value) {
  ...
  try {
    ...
    endpointForm.value = { displayName: endpointDetail.value.displayName, baseUrl: endpointDetail.value.baseUrl, enabled: endpointDetail.value.enabled, apiToken: '', clearApiToken: false, inputPricePerMillion: endpointDetail.value.inputPricePerMillion ?? 0, outputPricePerMillion: endpointDetail.value.outputPricePerMillion ?? 0, currency: endpointDetail.value.currency ?? 'KRW', clearPricing: endpointDetail.value.inputPricePerMillion == null && endpointDetail.value.outputPricePerMillion == null }
