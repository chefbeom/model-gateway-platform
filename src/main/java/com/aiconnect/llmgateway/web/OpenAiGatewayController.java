...
public class OpenAiGatewayController {
    ...
    public OpenAiGatewayController(ChatCompletionGateway gateway, ApiKeyService apiKeyService, LlmServiceRepository services,
                                   ProjectServiceAccessRepository access, ObjectMapper objectMapper) {
        this.gateway = gateway; this.apiKeyService = apiKeyService; this.services = services; this.access = access; this.objectMapper = objectMapper;
    }
    @PostMapping("/chat/completions")
    public ResponseEntity<JsonNode> chatCompletions(@RequestHeader(HttpHeaders.AUTHORIZATION) String authorization, @RequestBody JsonNode body) {
    ...
    @GetMapping("/models")
    public ObjectNode models(@RequestHeader(HttpHeaders.AUTHORIZATION) String authorization) {
        ApiKeyCredentials credentials = apiKeyService.authenticate(authorization);
        ObjectNode response = objectMapper.createObjectNode(); response.put("object", "list"); ArrayNode models = response.putArray("data");
        for (ProjectServiceAccess entitlement : access.findByIdProjectId(credentials.project().getId())) {
            services.findById(entitlement.getId().getServiceId()).filter(LlmService::isEnabled).ifPresent(service -> {
                ObjectNode model = models.addObject(); model.put("id", service.getServiceKey()); model.put("object", "model"); model.put("owned_by", "aiconnect");
            });
        }
        return response;
    }
