...
public class AdminController {
    ...
    public record DeploymentView(UUID id, UUID runtimeEndpointId, UUID externalProviderId, String providerModelId, String compatibilityKey, String displayName, String modelFamily, String quantization, Integer contextLength, boolean loaded, boolean enabled, HealthStatus healthStatus, int maxConcurrency, String capabilitiesJson, String capabilityOverridesJson, BigDecimal inputPricePerMillion, BigDecimal outputPricePerMillion, Currency currency) { static DeploymentView from(ModelDeployment item) { return new DeploymentView(item.getId(), item.getRuntimeEndpointId(), item.getExternalProviderId(), item.getProviderModelId(), item.getCompatibilityKey(), item.getDisplayName(), item.getModelFamily(), item.getQuantization(), item.getContextLength(), item.isLoaded(), item.isEnabled(), item.getHealthStatus(), item.getMaxConcurrency(), item.getCapabilitiesJson(), item.getCapabilityOverridesJson(), item.getProviderInputPricePerMillion(), item.getProviderOutputPricePerMillion(), item.getProviderPriceCurrency()); } }
