...
public class AdminController {
    ...
    public record EndpointView(UUID id, UUID nodeId, String displayName, RuntimeType runtimeType, String baseUrl, boolean enabled, HealthStatus healthStatus, Instant lastCheckedAt, BigDecimal inputPricePerMillion, BigDecimal outputPricePerMillion, Currency currency) { static EndpointView from(RuntimeEndpoint item) { return new EndpointView(item.getId(), item.getNodeId(), item.getDisplayName(), item.getRuntimeType(), item.getBaseUrl(), item.isEnabled(), item.getHealthStatus(), item.getLastCheckedAt(), item.getInputPricePerMillion(), item.getOutputPricePerMillion(), item.getCurrency()); } }
    public record DeploymentView(UUID id, UUID runtimeEndpointId, UUID externalProviderId, String providerModelId, String compatibilityKey, String displayName, String modelFamily, String quantization, Integer contextLength, boolean loaded, boolean enabled, HealthStatus healthStatus, int maxConcurrency, String capabilitiesJson, String capabilityOverridesJson, BigDecimal inputPricePerMillion, BigDecimal outputPricePerMillion, Currency currency, String metadataJson) { static DeploymentView from(ModelDeployment item) { return new DeploymentView(item.getId(), item.getRuntimeEndpointId(), item.getExternalProviderId(), item.getProviderModelId(), item.getCompatibilityKey(), item.getDisplayName(), item.getModelFamily(), item.getQuantization(), item.getContextLength(), item.isLoaded(), item.isEnabled(), item.getHealthStatus(), item.getMaxConcurrency(), item.getCapabilitiesJson(), item.getCapabilityOverridesJson(), item.getProviderInputPricePerMillion(), item.getProviderOutputPricePerMillion(), item.getProviderPriceCurrency(), item.getMetadataJson()); } }
    public record ServiceView(UUID id, UUID organizationId, String serviceKey, String displayName, BigDecimal inputPricePerMillion, BigDecimal outputPricePerMillion, Currency currency, FailoverPolicy failoverPolicy, RetryPolicy retryPolicy, boolean allowDegraded, boolean enabled, TemperaturePolicy temperaturePolicy, BigDecimal temperatureValue, ReasoningEffort reasoningEffort, boolean openAiFastMode) { static ServiceView from(LlmService item) { return new ServiceView(item.getId(), item.getOrganizationId(), item.getServiceKey(), item.getDisplayName(), item.getInputPricePerMillion(), item.getOutputPricePerMillion(), item.getCurrency(), item.getFailoverPolicy(), item.getRetryPolicy(), item.isAllowDegraded(), item.isEnabled(), item.getTemperaturePolicy(), item.getTemperatureValue(), item.getReasoningEffort(), item.isOpenAiFastMode()); } }
    public record TargetView(UUID id, UUID serviceId, UUID deploymentId, int priority, boolean degraded,
                             boolean followModelChanges) { static TargetView from(ServiceTarget item) { return new TargetView(item.getId(), item.getServiceId(), item.getDeploymentId(), item.getPriority(), item.isDegraded(), item.isFollowModelChanges()); } }
