...
public class ControlPlaneService {
    ...
    public ControlPlaneService(OrganizationRepository organizations, ProjectRepository projects, InferenceNodeRepository nodes,
                               RuntimeEndpointRepository endpoints, ModelDeploymentRepository deployments, LlmServiceRepository services,
                               ProjectServiceAccessRepository access, ServiceTargetRepository targets, SecretCipher secretCipher,
                               InferenceRuntimeClient runtimeClient, LmStudioModelDiscovery modelDiscovery) {
        this.organizations = organizations; this.projects = projects; this.nodes = nodes; this.endpoints = endpoints;
        this.deployments = deployments; this.services = services; this.access = access; this.targets = targets;
        this.secretCipher = secretCipher; this.runtimeClient = runtimeClient; this.modelDiscovery = modelDiscovery;
    }

    @Transactional public Organization create(AdminDtos.CreateOrganization request) { return organizations.save(new Organization(request.name())); }
    @Transactional public Project create(AdminDtos.CreateProject request) {
        requireOrganization(request.organizationId()); return projects.save(new Project(request.organizationId(), request.name()));
    }
    @Transactional public InferenceNode create(AdminDtos.CreateNode request) {
        requireOrganization(request.organizationId());
        return nodes.save(new InferenceNode(request.organizationId(), request.name(), request.description(), defaulted(request.connectionMode(), "DIRECT"), request.labelsJson()));
    }
    @Transactional public RuntimeEndpoint create(AdminDtos.CreateEndpoint request) {
    ...
    @Transactional public ModelDeployment create(AdminDtos.CreateDeployment request) {
    ...
    @Transactional public LlmService create(AdminDtos.CreateService request) {
        requireOrganization(request.organizationId());
    ...
    @Transactional public void grantAccess(UUID projectId, UUID serviceId) {
        Project project = requireProject(projectId); LlmService service = requireService(serviceId);
        if (!project.getOrganizationId().equals(service.getOrganizationId())) throw new ApiException(HttpStatus.BAD_REQUEST, "ORGANIZATION_MISMATCH", "A project can only access a service in its organization.");
        access.save(new ProjectServiceAccess(projectId, serviceId));
    }
    @Transactional public ServiceTarget addTarget(UUID serviceId, AdminDtos.CreateTarget request) {
        LlmService service = requireService(serviceId); ModelDeployment deployment = requireDeployment(request.deploymentId());
        RuntimeEndpoint endpoint = requireEndpoint(deployment.getRuntimeEndpointId()); InferenceNode node = requireNode(endpoint.getNodeId());
        if (!service.getOrganizationId().equals(node.getOrganizationId())) throw new ApiException(HttpStatus.BAD_REQUEST, "ORGANIZATION_MISMATCH", "A service target must belong to the service organization.");
        return targets.save(new ServiceTarget(serviceId, deployment.getId(), request.priority(), request.weight() == null ? 100 : request.weight(), request.degraded(), request.maxConcurrencyOverride()));
    }
    @Transactional public ProbeResult probe(UUID endpointId) {
        ...
        try {
            ...
            endpoint.recordHealth(healthy); endpoints.save(endpoint);
            ...
            endpoint.recordHealth(false); endpoints.save(endpoint); syncDeploymentHealth(endpoint, List.of(), false);
            return new ProbeResult(false, 0, List.of());
        }
    }
    @Transactional public List<ModelDeployment> syncModels(UUID endpointId) {
        RuntimeEndpoint endpoint = requireEndpoint(endpointId);
        RuntimeResult result = runtimeClient.listModels(endpoint);
        if (!result.isSuccessful()) throw new ApiException(HttpStatus.BAD_GATEWAY, "RUNTIME_PROBE_FAILED", "The runtime rejected the model-list request.");
        endpoint.recordHealth(true); endpoints.save(endpoint);
        ...
        for (ModelDeployment deployment : existing.values()) {
            if (!seen.contains(deployment.getProviderModelId())) {
                deployment.markUnavailable(); deployments.save(deployment);
            }
        }
        return created;
    }
    ...
    private Organization requireOrganization(UUID id) { return organizations.findById(id).orElseThrow(() -> notFound("ORGANIZATION_NOT_FOUND", "The organization does not exist.")); }
    ...
    private InferenceNode requireNode(UUID id) { return nodes.findById(id).orElseThrow(() -> notFound("NODE_NOT_FOUND", "The inference node does not exist.")); }
    ...
    private LlmService requireService(UUID id) { return services.findById(id).orElseThrow(() -> notFound("SERVICE_NOT_FOUND", "The logical service does not exist.")); }
